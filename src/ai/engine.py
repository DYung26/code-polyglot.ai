...
from src.ai.prompts import build_instruction_conversion_prompt, build_instruction_prompt, build_translation_prompt
...
class AIEngine:
    ...
    def instruction_conversion_module(self, text: str, current_lang: str, target_langs: List[str]) -> str:
        prompt = build_instruction_conversion_prompt(text, current_lang, target_langs[0])
        # print("Prompt:\n", prompt)
        response = self.client.responses.create(
            model=self.model,
            input=prompt,
